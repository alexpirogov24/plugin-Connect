...
include(MRA_IMPORT_PSQL_DIR . "/cron_functions.php");
...
function cron_mra_psql_event_func(){
	...
	try{ 
	   ...
	   //  		$text_log .= date("Y-m-d H:i:s").": outlet_stock ".print_r(empty($outlet_stock_st), true)." \r\n";
            ...
            /*
                //// Checking the price of a product for additional parameters ////
                //$price_m_e = get_post_meta( $product->ID, '_price', true );
                $price_m_e = get_post_meta( $product->ID, '_wc_cog_cost', true );
                $map_m_e = get_post_meta($product->ID, '_minimum_advertised_price', true);

                if($map_m_e > 0) {
                	update_post_meta($product->ID, '_regular_price', $map);
    				update_post_meta($product->ID, '_price', $map);
            	} else {
	                $new_price_m_e = apply_custom_markup_or_exclusion($price_m_e, $map_m_e, $product->ID);

	                if ($price_m_e != $new_price_m_e) {
	                    $text_log = date("Y-m-d H:i:s").": The price was changed according to the “Custom Markup” or “Exclusions” parameters in product ".$product->post_title." (".$product->ID."). \r\n";
	                    $result_fwrite = fwrite($log_file, $text_log);
	                }
                }
            */
            ...
            if ($mra_import_psql_enable_nfa == 'on') {
                $result_nfa_tag = mra_check_and_add_nfa_tag($product->ID);
                if ($result_nfa_tag) {
                    $text_log = date("Y-m-d H:i:s").": ".$result_nfa_tag." \r\n";
                    $result_fwrite = fwrite($log_file, $text_log);
                }
            }
